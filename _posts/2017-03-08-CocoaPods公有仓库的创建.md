...
> 本文发布于 [ZY Blog](http://qiubaiying.github.io)、[简书](http://www.jianshu.com/p/d2d98298b1b8) 转载请保留链接
...
下面我们将一步步把我封装的这个简单的TextFiled控件 [ZYPhoneNumTF](https://github.com/qiubaiying/ZYPhoneNumTF) 上传到 Cocoapods 公有仓库中。
...
- `README.md`:仓库说明
- 你的代码
- `ZYPhoneNumTF.podspec`: CocoaPods 的描述文件，这个文件**非常重要**

如下图：

![](https://ww2.sinaimg.cn/large/006tNbRwgy1fdfhvy3c19j31iq0dqn03.jpg)

`ZYPhoneNumTF` 文件夹下是我存放代码的地方

`ZYPhoneNumTF_Demo` 是代码使用样例（不是必须的）
...
在你的仓库目录下，使用终端命令创建
	...
	$ pod spec create ZYPhoneNumTF
	
这时就会在你的仓库下生成 `ZYPhoneNumTF.podspec` 文件
...
```ruby
Pod::Spec.new do |s|
  s.name         = "ZYPhoneNumTF" # 项目名称
  s.version      = "1.0.0"        # 版本号 与 你仓库的 标签号 对应
  s.license      = "MIT"          # 开源证书
  s.summary      = "A delightful TextField of PhoneNumber" # 项目简介

  s.homepage     = "https://github.com/qiubaiying/ZYPhoneNumTF" # 你的主页
  s.source       = { :git => "https://github.com/qiubaiying/ZYPhoneNumTF.git", :tag => "#{s.version}" }#你的仓库地址，不能用SSH地址
  s.source_files = "ZYPhoneNumTF/*.{h,m}" # 你代码的位置， ZYPhoneNumTF/*.{h,m} 表示 ZYPhoneNumTF 文件夹下所有的.h和.m文件
  ...
  # User
  s.author             = { "ZY" => "qiubaiyingios@163.com" } # 作者信息
...
验证会出现成功出现
	...
	ZYPhoneNumTF passed validation.	
...
但是很多情况没这么顺利，比如:
	...
	[!] ZYPhoneNumTF did not pass validation, due to 1 warning (but you can use `--allow-warnings` to ignore it) and all results apply only to public specs, but you can use `--private` to ignore them if linting the specification for a private pod.
...
最后一步，发布项目的描述的文件 `ZYPhoneNumTF.podspec` 

在仓库目录下执行
	
	pod trunk push ZYPhoneNumTF.podspec
	
将你的 `ZYPhoneNumTF.podspec` 发布到公有的speecs上,这一步其实做了很多操作,包括 
...
成功后将会出现下列信息：
	...
	--------------------------------------------------------------------------------
	 ...
	 🚀  ZYPhoneNumTF (1.0.0) successfully published
	 📅  March 7th, 01:39
	 🌎  https://cocoapods.org/pods/ZYPhoneNumTF
	 👍  Tell your friends!
	 
说明发布成功，你就可以通过上面的URL: <https://cocoapods.org/pods/ZYPhoneNumTF> 进入的Pods查看自己的仓库信息了.
...
查询仓库
	...
	$ pod search ZYPhoneNumTF
---
	-> ZYPhoneNumTF (1.0.0)
	   A delightful TextField of PhoneNumber
	   pod 'ZYPhoneNumTF', '~> 1.0.0'
	   - Homepage: https://github.com/qiubaiying/ZYPhoneNumTF
	   - Source:   https://github.com/qiubaiying/ZYPhoneNumTF.git
	   - Versions: 1.0.0, 0.0.1 [ZYPhoneNumTF repo]
	(END)

若出现仓库信息说明已经成功了，这时候你就可以在 `Podfile` 添加、使用自己的仓库了 `pod 'ZYPhoneNumTF', '~> 1.0.0'`
...
- 更新`ZYPhoneNumTF.podspec`中的版本号
- 打上标签推送远程
- `pod trunk push ZYPhoneNumTF.podspec` 推送到pods仓库
